define(['jquery', 'backbone', 'handlebars', 'lu-model-state'],
function ($, Backbone, Handlebars, appState) {
    ...
    return Backbone.View.extend({
        ...
        getPagesTotal: function () {
            // return appState.get('readium').reader.getLoadedSpineItems().length;
            return this.positionInfo.totalChapters;
        },

        setPageNum: function (pageNum) {
            //return appState.get('readium').reader.openPageIndex(pageNum - 1);
            var reader = appState.get('readium').reader;
